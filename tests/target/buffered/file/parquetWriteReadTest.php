...
class parquetWriteReadTest extends abstractWriteReadTest
{
    ...
    public function testOtherDatatypesUsingAutoguess(): void
    {
        if (!extension_loaded('gmp')) {
            static::fail('GMP extension required for working with arbitrary precision numbers in Parquet.');
        }
    ...
    protected function compareData(
        ...
        static::assertCount(count($samples), $result);
        ...
        foreach ($samples as $index => $sample) {
            ...
            foreach ($sample as $key => $value) {
                if (is_numeric($value) && is_string($value)) {
                    ...
                    static::assertEquals(0, bccomp($value, $resultRow[$key]), 24);
                } else {
                    static::assertEquals($value, $resultRow[$key]);
                }
            }
        }
    }
