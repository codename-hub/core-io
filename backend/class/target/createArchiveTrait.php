...
trait createArchiveTrait
{
  ...
  protected function createArchive() : array {
    ...
    foreach($this->fileResults as $fileResult) {
      ...
      if($fileResult instanceof \codename\core\io\value\text\fileabsolute\tagged) {
        $tags = $fileResult->getTags();
        $archiveTarget = $fileResult->getTags()[0]['archive_name'];
    ...
    foreach($tempZipInstances as $archiveTarget => $instance) {
      ...
      if($tags) {
        ...
        $zipFileResult = new \codename\core\io\value\text\fileabsolute\tagged($filename, $tags);
